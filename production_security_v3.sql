...
-- Execute this file in the Supabase SQL Editor BEFORE deploying the matching
-- frontend branch. It replaces the legacy PIN/localStorage authorization model.
-- =============================================================================

BEGIN;

-- -----------------------------------------------------------------------------
-- 1. Staff identities are real Supabase Auth users.
--    Create staff accounts in Authentication > Users, then insert their user_id
--    into this table. Never expose service_role keys to the browser.
-- -----------------------------------------------------------------------------
...
-- Helper used by RLS policies. It only returns whether the current authenticated
-- Supabase user is an active staff member. It does not trust client metadata.
...
-- -----------------------------------------------------------------------------
-- 2. Orders: bind customer orders to the authenticated/anonymous Auth identity.
-- -----------------------------------------------------------------------------
...
-- Legacy PIN auth is intentionally removed.
DROP FUNCTION IF EXISTS public.verify_admin_pin(TEXT);
ALTER TABLE public.store_settings DROP COLUMN IF EXISTS admin_pin;

-- -----------------------------------------------------------------------------
-- 3. Remove every legacy policy from the application tables so no permissive
--    policy such as USING (true) remains active by accident.
-- -----------------------------------------------------------------------------
DO $$
DECLARE
  p RECORD;
...
-- -----------------------------------------------------------------------------
-- 4. Grants + RLS: public catalog is read-only; staff can manage it.
-- -----------------------------------------------------------------------------
...
-- Product cost price is not granted to public/customer sessions.
...
-- Admin-only RPC used to retrieve cost_price without exposing it to customers.
...
-- Neighborhood delivery configuration is readable, but only admin may mutate it.
...
-- Store settings contain only public operational data after admin_pin removal.
...
-- -----------------------------------------------------------------------------
-- 5. Orders: customers may only create/read their own orders. Staff may operate
--    all orders. Customers have no direct UPDATE permission through RLS.
-- -----------------------------------------------------------------------------
...
-- Customer Pix proof attachment goes through this narrow RPC. Customers never
-- receive a general UPDATE policy on orders.
...
BEGIN
  ...
  -- Uploaded files are namespaced by the authenticated uploader.
...
-- Catalog images remain public to read, but only admins can write them.
...
-- Optional cashier table: staff-only if present.
DO $$
BEGIN
  IF to_regclass('public.cashier_logs') IS NOT NULL THEN
...
-- =============================================================================
-- REQUIRED ONE-TIME SETUP AFTER RUNNING THIS MIGRATION
-- 1) Supabase Dashboard > Authentication > Providers > enable Anonymous Sign-Ins.
-- 2) Create an email/password staff user in Authentication > Users.
-- 3) Promote it with:
--
...
--
-- 4) Sign out/in after changing a staff account.
-- =============================================================================
